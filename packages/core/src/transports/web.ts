...
export class WebSerialTransport implements Transport {
  ...
  static async request(filters: WebSerialFilter[] = []): Promise<WebSerialTransport> {
    if (!WebSerialTransport.isSupported()) {
      throw new AwgError("Web Serial API is not supported in this browser");
    }
  ...
  async open(options: SerialOpenOptions): Promise<void> {
    ...
    if (!this.port.readable || !this.port.writable) {
      throw new AwgError("Port has no readable/writable streams");
    }
  ...
  async write(data: Uint8Array | string): Promise<void> {
    if (!this.writer) throw new AwgError("Port not open");
