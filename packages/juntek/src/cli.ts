#!/usr/bin/env node
/**
 * `juntek` command-line tool (Node-only).
 *
 *   juntek devices                        # supported drivers + verification status
 *   juntek list                           # serial ports
 *   juntek set --device jds6600 --freq 727.5 --amp 20 --on
 *   juntek set --device jds6600 --off
 *
...
import { JUNTEK_DEVICES } from "./devices.js";
...
export function parseCliArgs(argv: string[]): ParsedCli {
  ...
  if (!options) {
    throw new AwgError(`Unknown command "${command}" — run \`juntek --help\``);
  }
...
async function cmdSet(values: ParsedCli["values"]): Promise<void> {
  ...
  if (!deviceId) {
    throw new AwgError("--device is required — run `juntek devices` to see the options");
  }
  const path = values["port"] ? String(values["port"]) : undefined;
  if (!path) {
    throw new AwgError("--port is required — run `juntek list` to find it");
  }
