...
import { limitsForModel, traitsForModel } from "./limits.js";
...
/**
 * Is this string one of the generic waveform kinds, rather than an FY-specific
 * name like "Rectangle" or "Arbitrary3"?
 *
 * The two vocabularies only overlap on names that resolve to the same code
 * either way (`"sine"`, `"square"` on the FY6900 family), so preferring the
 * generic reading never changes an existing result — it only makes kinds that
 * used to throw (`"ramp-up"`, `"noise"`) work.
 */
const WAVEFORM_KINDS: readonly WaveformKind[] = [
  "sine",
  "square",
  "triangle",
  "ramp-up",
...
function isWaveformKind(value: string): value is WaveformKind {
  return (WAVEFORM_KINDS as readonly string[]).includes(value);
}
...
export class FeelTech implements SignalGenerator {
  ...
  /** Vendor, model and reported ID, in the vendor-neutral shape. */
  get info(): DeviceInfo {
    return {
      vendor: "FeelTech",
      model: this.model || this.detectedFamily,
      ...(this.id ? { reportedId: this.id } : {}),
    };
  }

  /**
   * What this device can do, as far as we can establish it.
   *
   * Only meaningful after `open()` — the model string that drives the limits
   * and the duty-gating trait comes from the device itself. Before that, the
   * limits report as unknown rather than as a default.
   */
  get capabilities(): Capabilities {
    const traits = traitsForModel(this.model, this.detectedFamily);
    return {
      ...DEFAULT_CAPABILITIES,
      channels: traits.channels,
      dutyGatedByWaveform: traits.dutyGatedByWaveform,
      // FY devices answer RM*/RF* reads for every parameter — this is exactly
      // what the `verifyWrites` retry loop depends on.
      readback: true,
      waveforms: supportedKinds(this.detectedFamily),
      limits: limitsForModel(this.model),
    };
  }
   ...
   * - a generic {@link WaveformKind} (`"sine"`, `"ramp-down"`, …), mapped to
   *   this family's table — the portable form,
   * - an FY waveform name (`"Rectangle"`, `"NegRamp"`, `"Arbitrary3"`),
   * - a raw numeric code.
   *
   * Returns what the device will actually output. On the FY series that always
   * matches the request except for `"dc"` on the FY2300, whose table has no
   * constant-level entry; the returned `substituted` flag says so rather than
   * swapping shapes behind your back.
   */
  async setWaveform(
    channel: Channel,
    waveform: WaveformKind | number | string,
  ): Promise<AppliedWaveform> {
    const applied =
      typeof waveform === "string" && isWaveformKind(waveform)
        ? resolveWaveformKind(this.detectedFamily, channel, waveform)
        : ((code: number): AppliedWaveform => {
            const kind = kindForCode(this.detectedFamily, channel, code);
            return { requested: kind, actual: kind, substituted: false, code };
          })(resolveWaveform(this.detectedFamily, channel, waveform));

    await this.setVerified(
      `set waveform ${waveform}`,
      () =>
        this.sendWrite(chCode(channel, "W"), applied.code.toString().padStart(2, "0")),
      () => this.getWaveform(channel),
      (got) => got === applied.code,
    );
    return applied;
  }
  ...
  /**
   * Apply a complete channel state, in the vendor-neutral {@link ChannelStep}
   * shape.
   *
   * The FY series accepts parameters in any order, so this is a thin rename
   * over {@link configureChannel} and emits exactly the same wire traffic.
   */
  async applyStep(channel: Channel, step: ChannelStep): Promise<void> {
    await this.configureChannel(channel, {
      ...(step.waveform !== undefined ? { waveform: step.waveform } : {}),
      ...(step.frequencyHz !== undefined ? { frequencyHz: step.frequencyHz } : {}),
      ...(step.amplitudeVpp !== undefined ? { amplitudeV: step.amplitudeVpp } : {}),
      ...(step.offsetV !== undefined ? { offsetV: step.offsetV } : {}),
      ...(step.dutyCyclePct !== undefined ? { dutyCyclePct: step.dutyCyclePct } : {}),
      ...(step.phaseDeg !== undefined ? { phaseDeg: step.phaseDeg } : {}),
      ...(step.output !== undefined ? { enabled: step.output } : {}),
    });
  }
