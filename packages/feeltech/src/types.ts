 ...
 *
 * These are aliases of the shared `@freqgen/core` error types, not
 * subclasses — the same class objects under the historical names — so
 * `instanceof FeelTechError` and `instanceof AwgError` both hold, whichever
 * layer threw. The one visible change from pre-0.2 releases is `err.name`,
 * which now reads `"AwgError"` rather than `"FeelTechError"`.
 */
export {
  AwgError as FeelTechError,
  AwgTimeoutError as FeelTechTimeoutError,
  AwgProtocolError as FeelTechProtocolError,
  AwgVerifyError as FeelTechVerifyError,
} from "@freqgen/core";
