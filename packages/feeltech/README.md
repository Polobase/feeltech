...
├── waveforms.ts       # waveform tables, resolver & generic-kind mapping
...
│   └── web.ts         # re-exports @freqgen/core/web
└── types.ts           # enums, error aliases, result types
...
`applyStep()` is the portable call — on the FY series it is a rename of `configureChannel()` emitting identical wire traffic, while generators with a required command ordering implement it as their native sequence.

| Package | Devices |
| --- | --- |
| `@freqgen/juntek` | JUNTEK JDS6600/2800/2900/8000, Koolertron CJDS66 & MHS-5200A |
| `@freqgen/spooky2` | Spooky2 XM, Gen X, Gen X Pro |
| `@freqgen/core` | shared transports, capabilities, limits, octave folding |

### The FY3200S is a separate driver

`FY3200S` covers the FY3200S/FY3224S, which share a manufacturer with the rest of
this package and nothing else — 9600 8N1, lowercase commands, a `b`/`d` channel
prefix, centihertz, and **no output relay**, so `setOutput()` is emulated by
writing the amplitude. It reports `capabilities.outputRelay === false`. Not
verified against hardware.

### Capabilities and limits

```ts
const { limits, waveforms, dutyGatedByWaveform } = fy.capabilities;
```
